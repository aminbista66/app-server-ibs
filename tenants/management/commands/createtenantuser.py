...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--tenant", help="Specify Tenant for assigning groups")
    ...
    def handle(self, *args, **options):
        ...
        try:
            if options["interactive"]:
                ...
                # foreign keys as fake modeles instead of raw IDs.
                fake_user_data = {}
            ...
            password = user_data.pop(PASSWORD_FIELD)

            try:
                subscription = Subscription.objects.using("default").get(
                    tenant__schema_name=database
                )
            except Exception as e:
                raise CommandError('No subscription found for given tenant')
 
            try:
                subscription_group = Group.objects.using("default").get(
                    name=subscription.group.name
                )
            except Exception as e:
                raise CommandError('Group for user not found')

            try:
                tenant_group, is_created = Group.objects.using(database).get_or_create(name='Admin Group')
                tenant_permission_list = list(subscription_group.permissions.all())
            except Exception as e:
                raise CommandError('Group for tenant cannot be created')

            try:
                tenant_group.permissions.set(tenant_permission_list)
                tenant_group.save()
            except Exception as e:
                raise CommandError('Cannot set permission for tenant\'s admin')

            try:
                user: User = self.UserModel._default_manager.db_manager(
                    database
                ).create(**user_data, is_tenantadmin=True)
                user.set_password(password)
                user.groups.add(tenant_group)
                user.save()
            except Exception as e:
                raise CommandError("user cannot be created")
